...
export default function IntroAnimation() {
    ...
    // 1. Morph Progress: Circle stays (no morphing to arc)
    // Keep circle formation, don't morph to arc
    const morphProgress = useMotionValue(0);
    const smoothMorph = useSpring(morphProgress, { stiffness: 40, damping: 20 });

    // Remove scroll rotation - no arc animation needed
    ...
    // --- Content Opacity ---
    // Content stays visible (no fade in based on morph)
    const contentOpacity = useMotionValue(1);
    const contentY = useMotionValue(0);

    // --- Fade Out Effect ---
    // Fade out quickly when user scrolls (scroll 0-300)
    const fadeOutOpacity = useTransform(virtualScroll, [0, 300], [1, 0]);
    const fadeOutScale = useTransform(virtualScroll, [0, 300], [1, 0.9]);
