...
/**
 * Computes the fan-spread offset for a card at `index` when card `hoveredIndex`
 * is being hovered.  The two immediate neighbours fan outward; cards further
 * away stay put.  Mirrors the translate logic from the twitter-testimonial-cards.
 */
function getFanOffset(index: number, hoveredIndex: number): { dx: number; dy: number } {
    if (hoveredIndex < 0) return { dx: 0, dy: 0 };

    const dist = index - hoveredIndex;

    // Direct left/right neighbours fan away from the hovered card
    if (dist === 1)  return { dx:  22, dy:  14 };
    if (dist === -1) return { dx: -22, dy:  14 };
    if (dist === 2)  return { dx:  38, dy:  24 };
    if (dist === -2) return { dx: -38, dy:  24 };
...
function FlipCard({
    ...
    const { dx, dy } = getFanOffset(index, hoveredIndex);
    const isHovered = hoveredIndex === index;

    return (
        <motion.div
            // Position driven by intro phase
            animate={{
                x: target.x + dx,
                y: target.y + dy,
                rotate: target.rotation,
                scale: isHovered ? (target.scale || 1) * 1.18 : target.scale,
                opacity: target.opacity,
                zIndex: isHovered ? 50 : 1,
            }}
            transition={{
                type: "spring",
                stiffness: isHovered ? 300 : 60,
            ...
            onHoverEnd={onLeave}
        >
            {/* Card shell — grayscale dimmed by default, full colour on hover (mirrors testimonial style) */}
            <motion.div
                className="relative h-full w-full rounded-xl overflow-hidden shadow-lg"
                animate={{
                    filter: isHovered ? "grayscale(0%)" : "grayscale(20%)",
                    boxShadow: isHovered
                        ? "0 0 0 2px #e8944a, 0 8px 32px rgba(201,123,58,0.45)"
            ...
            >
                ...
                {/* Dim overlay on non-hovered cards (like the testimonial `before:` overlay) */}
                <motion.div
