...
function FlipCard({
    ...
    return (
        <motion.div
            // Start at scatter position so first animation is a fly-in
            initial={{
                x: SCATTER_POSITIONS[index].x,
                y: SCATTER_POSITIONS[index].y,
                rotate: SCATTER_POSITIONS[index].rotation,
                scale: SCATTER_POSITIONS[index].scale,
                opacity: 1,
            }}
...
// Stable seeded pseudo-random scatter positions (avoids hydration mismatch)
const SCATTER_POSITIONS = IMAGES.map((_, i) => {
    // Simple deterministic values based on index
    const angle = (i / IMAGES.length) * Math.PI * 2 + i * 0.9;
    const radius = 400 + (i % 5) * 120;
    return {
...
export default function IntroAnimation() {
    // Start at "line" so cards are immediately visible; scatter plays as initial -> animate
    ...
    // --- Intro Sequence ---
    useEffect(() => {
        // scatter -> line (cards fly in from edges) -> circle
